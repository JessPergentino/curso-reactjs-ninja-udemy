...
it('initialState should not be undefined', () => {
  try {
    ...
    expect(error.message).to.be.equal('initialState should not be undefined')
  }
})
...
it('handleActions should not be different from object', () => {
  try {
    ...
    expect(error.message).to.be.equal('createReducer expects the second argument as an objetc representing reducer')
  }
})
